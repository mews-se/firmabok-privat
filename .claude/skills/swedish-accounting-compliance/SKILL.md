...
This installation additionally sanctions direct edit (`edit_posted_entry`) and deletion (`delete_voucher`) of posted entries as operator-responsibility paths: the generic audit log records the change, but there is no rättelse ceremony. The two BFL tracks above remain the traceable options, and period locks apply to all paths alike.
...
These are the law's requirements. This installation does not enforce them: documents can be deleted via `delete_document` regardless of age, and `retention_expires_at` is informational. Meeting BFL 7 kap is the operator's own responsibility.
