  ...
  const handleRemoveConfirmed = async (doc: DocumentRecord) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        toast({
          title: body?.error ?? tj('remove_failed'),
          variant: 'destructive',
        })
        return
      }
      ...
      toast({ title: tj('remove_failed'), variant: 'destructive' })
    }
  }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] w-[95vw] max-w-4xl overflow-y-auto">
        ...
        <DestructiveConfirmDialog
          ...
          title={tj('remove_confirm_title')}
          description={tj('remove_confirm_body', { file: removeDoc?.file_name ?? '' })}
          confirmLabel={tj('remove_confirm_cta')}
