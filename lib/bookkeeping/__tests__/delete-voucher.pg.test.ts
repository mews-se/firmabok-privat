...
async function insertSequence(params: {
  userId: string
  companyId: string
  fiscalPeriodId: string
  lastNumber: number
  ...
  await getPool().query(
    ...
    [params.userId, params.companyId, params.fiscalPeriodId, params.lastNumber],
  )
}

async function readSequence(client: PoolClient, companyId: string, fiscalPeriodId: string): Promise<number> {
...
describe('delete_voucher.pg: RPC + immutability trigger interaction', () => {
  it('deletes the last posted voucher and decrements the sequence counter', async () => {
    const { userId, companyId, fiscalPeriodId } = await seedCompany()
    await insertPostedEntryWithLines({
      userId, companyId, fiscalPeriodId, voucherNumber: 1,
    })
    const lastId = await insertPostedEntryWithLines({
      userId, companyId, fiscalPeriodId, voucherNumber: 2,
    })
    await insertSequence({ userId, companyId, fiscalPeriodId, lastNumber: 2 })

    await withUserContext(userId, async (client) => {
      const r = await client.query<{ delete_voucher: { deleted: boolean; gap_created: boolean } }>(
        `SELECT public.delete_voucher($1::uuid, $2::uuid)`,
        [companyId, lastId],
      )
      expect(r.rows[0]!.delete_voucher.deleted).toBe(true)
      expect(r.rows[0]!.delete_voucher.gap_created).toBe(false)
      ...
      expect(await readSequence(client, companyId, fiscalPeriodId)).toBe(1)
    })
  })

  it('deletes a mid-series voucher, leaves the counter alone and reports the gap', async () => {
    const { userId, companyId, fiscalPeriodId } = await seedCompany()
    await insertPostedEntryWithLines({
      userId, companyId, fiscalPeriodId, voucherNumber: 1,
    })
    const midId = await insertPostedEntryWithLines({
      userId, companyId, fiscalPeriodId, voucherNumber: 2,
    })
    await insertPostedEntryWithLines({
      userId, companyId, fiscalPeriodId, voucherNumber: 3,
    })
    await insertSequence({ userId, companyId, fiscalPeriodId, lastNumber: 3 })
    ...
    await withUserContext(userId, async (client) => {
      ...
      expect(r.rows[0]!.delete_voucher.deleted).toBe(true)
      expect(r.rows[0]!.delete_voucher.gap_created).toBe(true)
      ...
      expect(after.rowCount).toBe(0)
      expect(await readSequence(client, companyId, fiscalPeriodId)).toBe(3)

      // The gap is real and detectable, but nothing requires explaining it.
      const gaps = await client.query(
        `SELECT * FROM public.detect_voucher_gaps($1::uuid, $2::uuid, 'A')`,
        [companyId, fiscalPeriodId],
      )
  ...
  it('refuses to delete a reversed original while its storno exists', async () => {
    const { userId, companyId, fiscalPeriodId } = await seedCompany()
    ...
    await withUserContext(userId, async (client) => {
      await expect(
        client.query(`SELECT public.delete_voucher($1::uuid, $2::uuid)`, [companyId, originalId]),
      ).rejects.toThrow(/reversed entry — delete its storno first/i)
    })
  })

  it('refuses to delete a posted voucher a correction references', async () => {
    const { userId, companyId, fiscalPeriodId } = await seedCompany()
    const originalId = await insertPostedEntryWithLines({
      userId, companyId, fiscalPeriodId, voucherNumber: 1,
    })
    ...
    await getPool().query(
      `INSERT INTO public.journal_entries
         (id, user_id, company_id, fiscal_period_id, voucher_number, voucher_series,
          entry_date, description, source_type, status, correction_of_id)
       VALUES ($1, $2, $3, $4, 2, 'A', '2026-06-01', 'Correction', 'correction', 'draft', $5)`,
      [correctionId, userId, companyId, fiscalPeriodId, originalId],
    )
    await insertBalancedLines(correctionId)
    await getPool().query(
      `UPDATE public.journal_entries SET status = 'posted' WHERE id = $1`,
      [correctionId],
    )
    ...
    await withUserContext(userId, async (client) => {
      await expect(
        client.query(`SELECT public.delete_voucher($1::uuid, $2::uuid)`, [companyId, originalId]),
      ).rejects.toThrow(/other entries reference this voucher/i)
    })
  })
  ...
  it('refuses deletion in a locked fiscal period', async () => {
    const { userId, companyId, fiscalPeriodId } = await seedCompany()
    const entryId = await insertPostedEntryWithLines({
      userId, companyId, fiscalPeriodId, voucherNumber: 1,
    })
    ...
    await withUserContext(userId, async (client) => {
      await expect(
        client.query(`SELECT public.delete_voucher($1::uuid, $2::uuid)`, [companyId, entryId]),
  ...
  it('clears journal_entry_id on attached documents and deletes the voucher', async () => {
    ...
    // gnubok.allow_delete bypass. delete_voucher unlinks documents
  ...
  // journal_entry_id outside delete_voucher (no gnubok.allow_delete
  ...
  it('deletes a draft entry without touching the voucher series', async () => {
    ...
    await withUserContext(userId, async (client) => {
      const result = await client.query<{ delete_voucher: { deleted: boolean; was_draft: boolean } }>(
        `SELECT public.delete_voucher($1::uuid, $2::uuid)`,
        [companyId, draftId],
      )
      expect(result.rows[0]!.delete_voucher.deleted).toBe(true)
      expect(result.rows[0]!.delete_voucher.was_draft).toBe(true)
