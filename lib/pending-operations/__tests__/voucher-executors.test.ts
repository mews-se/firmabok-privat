...
describe('commitPendingOperation: delete_voucher', () => {
  it('happy path: calls the RPC with p_user_id and surfaces the jsonb verdict', async () => {
    const { supabase, enqueue } = createQueuedMockSupabase()
    enqueue({ data: { id: 'op-1' }, error: null }) // CAS claim
    enqueue({
      data: { deleted: true, voucher_series: 'A', voucher_number: 13, was_period_ib: false, gap_created: true },
      error: null,
    }) // delete_voucher RPC
    enqueue({ data: null, error: null }) // dispatcher's commit update

    const op = makePendingOp({
      operation_type: 'delete_voucher',
      params: { journal_entry_id: 'je-1' },
    })
    ...
    expect(supabase.rpc).toHaveBeenCalledWith('delete_voucher', {
  ...
  it('auto-rejects with 404 when the RPC reports the entry missing', async () => {
    const { supabase, enqueue } = createQueuedMockSupabase()
    enqueue({ data: { id: 'op-1' }, error: null }) // CAS claim
    enqueue({ data: null, error: { message: 'Journal entry not found' } }) // RPC error
    enqueue({ data: null, error: null }) // dispatcher's reject update

    const op = makePendingOp({
      operation_type: 'delete_voucher',
      params: { journal_entry_id: 'je-missing' },
    })
  ...
  it('auto-rejects with 409 when other entries still reference the voucher', async () => {
    const { supabase, enqueue } = createQueuedMockSupabase()
    enqueue({ data: { id: 'op-1' }, error: null }) // CAS claim
    enqueue({
      data: null,
      error: {
        message:
          'Cannot delete: other entries reference this voucher (1 references). Delete the referencing storno/correction first.',
      },
    })
    enqueue({ data: null, error: null }) // dispatcher's reject update
    ...
    const op = makePendingOp({
      operation_type: 'delete_voucher',
      params: { journal_entry_id: 'je-referenced' },
    })
  ...
  it('returns 400 when journal_entry_id is missing', async () => {
    ...
    const op = makePendingOp({ operation_type: 'delete_voucher', params: {} })
...
describe('commitPendingOperation: edit_posted_entry', () => {
  it('happy path: forwards description, entry_date and lines to the RPC', async () => {
    const { supabase, enqueue } = createQueuedMockSupabase()
    enqueue({ data: { id: 'op-1' }, error: null }) // CAS claim
    enqueue({
      data: {
    ...
    const op = makePendingOp({
      operation_type: 'edit_posted_entry',
      params: {
        journal_entry_id: 'je-1',
        description: 'Rättad beskrivning',
        entry_date: '2026-02-20',
    ...
    expect(supabase.rpc).toHaveBeenCalledWith('edit_posted_entry', {
      p_company_id: 'company-1',
      p_entry_id: 'je-1',
      p_description: 'Rättad beskrivning',
      p_entry_date: '2026-02-20',
      p_lines: lines,
      p_user_id: 'user-1',
    })
  })

  it('passes null for omitted fields (metadata-only edit)', async () => {
    const { supabase, enqueue } = createQueuedMockSupabase()
    enqueue({ data: { id: 'op-1' }, error: null }) // CAS claim
    enqueue({ data: { changed: true }, error: null }) // RPC
    enqueue({ data: null, error: null }) // dispatcher's commit update

    const op = makePendingOp({
      operation_type: 'edit_posted_entry',
      params: { journal_entry_id: 'je-1', description: 'Bara ny text' },
    })
  ...
  it('auto-rejects with 409 when the period is locked', async () => {
    const { supabase, enqueue } = createQueuedMockSupabase()
    enqueue({ data: { id: 'op-1' }, error: null }) // CAS claim
    enqueue({ data: null, error: { message: 'Perioden är stängd eller låst.' } }) // RPC error
    enqueue({ data: null, error: null }) // dispatcher's reject update

    const op = makePendingOp({
      operation_type: 'edit_posted_entry',
      params: { journal_entry_id: 'je-1', description: 'Nytt' },
    })
    ...
    expect(result.status).toBe('rejected')
    expect(result.http_status).toBe(409)
    expect(result.error).toMatch(/låst/i)
  })

  it('returns 400 when no change field is present', async () => {
    const { supabase, enqueue } = createQueuedMockSupabase()
    enqueue({ data: { id: 'op-1' }, error: null }) // CAS claim
    enqueue({ data: null, error: null }) // dispatcher's reject update
    ...
    expect(result.status).toBe('failed')
    expect(result.http_status).toBe(400)
    expect(result.error).toMatch(/ingen ändring/i)
  })
})

// ─── delete_document ────────────────────────────────────────────────

describe('commitPendingOperation: delete_document', () => {
  it('happy path: delegates to the app deletion flow (RPC + storage removal)', async () => {
    vi.mocked(deleteDocument).mockResolvedValueOnce({
      ok: true,
      document: { id: 'doc-1', file_name: 'kvitto.pdf' },
    })
    ...
    expect(result.status).toBe('committed')
    expect(result.data).toMatchObject({ document_id: 'doc-1', file_name: 'kvitto.pdf', deleted: true })
    // userId must reach the service so the RPC can attribute the service-role
    // call to the acting user.
    expect(deleteDocument).toHaveBeenCalledWith(expect.anything(), 'company-1', 'doc-1', 'user-1')
  })

  it('auto-rejects with the service verdict when deletion is blocked', async () => {
    vi.mocked(deleteDocument).mockResolvedValueOnce({
      ok: false,
      reason: 'blocked',
      status: 409,
    ...
    const op = makePendingOp({
      operation_type: 'delete_document',
      params: { document_id: 'doc-evidence' },
    })
    ...
    expect(result.auto_rejected).toBe(true)
    expect(result.http_status).toBe(409)
    expect(result.error).toMatch(/leveransbevis/i)
  })
  ...
  it('returns 400 when document_id is missing', async () => {
    ...
    const op = makePendingOp({ operation_type: 'delete_document', params: {} })
